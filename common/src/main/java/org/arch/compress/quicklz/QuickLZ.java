package org.arch.compress.quicklz;
...
public final class QuickLZ
{
	...
	{
		if (headerLen(source, off) == 9) return fast_read(source, off, 5, 4);
		else return fast_read(source, off, 2, 1);
	}

	static public long sizeCompressed(byte[] source, int off)
	{
		if (headerLen(source, off) == 9) return fast_read(source, off, 1, 4);
		else return fast_read(source, off, 1, 1);
	}

	private static void write_header(byte[] dst, int level,
	...
	public static byte[] compress(byte[] source, int off, int length, int level)
	{
		int src = 0;
		...
		if (level == 1) hashtable = new int[HASH_VALUES][QLZ_POINTERS_1];
		else hashtable = new int[HASH_VALUES][QLZ_POINTERS_3];

		if (length == 0) return new byte[0];

		if (src <= last_matchstart)
		    fetch = (int) fast_read(source, off, src, 3);

		while (src <= last_matchstart)
		{
			if ((cword_val & 1) == 1)
			{
				if (src > 3 * (length >> 2) && dst > src - (src >> 5))
				{
					d2 = new byte[length + DEFAULT_HEADERLEN];
					write_header(d2, level, false, length, length
					        + DEFAULT_HEADERLEN);
					System.arraycopy(source, off, d2, DEFAULT_HEADERLEN, length);
					return d2;
				}
			...
			if (level == 1)
			{
				                ...
				                && source[src] == source[src + 1] && source[src] == source[src + 2])))
				{
					...
					if (source[off + o + 3] != source[off + src + 3])
					{
						int f = 3 - 2 | (hash << 4);
					...
					{
						int old_src = src;
						int remaining = ((length - UNCOMPRESSED_END - src + 1 - 1) > 255 ? 255
						        : (length - UNCOMPRESSED_END - src + 1 - 1));

						src += 4;
						if (source[off + o + src - old_src] == source[off + src])
						{
							src++;
							if (source[off + o + src - old_src] == source[off
							        + src])
							{
								src++;
								while (source[off + o + (src - old_src)] == source[off
								        + src]
				...
				else
				{
					lits++;
					hash_counter[hash] = 1;
					destination[dst] = source[off + src];
					cword_val = (cword_val >>> 1);
					src++;
					dst++;
					fetch = ((fetch >>> 8) & 0xffff)
					        | ((((int) source[off + src + 2]) & 0xff) << 16);
				}
			}
			else
			{
				...
				{
					o = hashtable[hash][k];
					if ((byte) fetch == source[off + o]
					        && (byte) (fetch >>> 8) == source[off + o + 1]
					        && (byte) (fetch >>> 16) == source[off + o + 2]
					        && o < src - MINOFFSET)
					{
						m = 3;
						while (source[off + o + m] == source[off + src + m]
						        && m < remaining)
							m++;
				...
				if (matchlen >= 3 && src - o < 131071)
				{
					...
					{
						fetch = (int) fast_read(source, off, src + u, 3);
	...
	static long fast_read(byte[] a, int off, int i, int numbytes)
	{
		long l = 0;
		for (int j = 0; j < numbytes; j++)
			l |= ((((int) a[off + i + j]) & 0xffL) << j * 8);
		return l;
	}
	...
	static public byte[] decompress(byte[] source, int off, int length)
	{
		int size = (int) sizeDecompressed(source, off);
		int src = headerLen(source, off);
		int dst = 0;
		long cword_val = 1;
		...
		if ((source[off] & 1) != 1)
		{
			...
			System.arraycopy(source, off + headerLen(source, off), d2, 0, size);
			return d2;
		}
		...
		for (;;)
		{
			if (cword_val == 1)
			{
				cword_val = fast_read(source, off, src, 4);
				src += 4;
				if (dst <= last_matchstart)
				{
					if (level == 1) fetch = (int) fast_read(source, off, src, 3);
					else fetch = (int) fast_read(source, off, src, 4);
				}
			}
			...
			if ((cword_val & 1) == 1)
			{
				...
				if (level == 1)
				{
					...
					{
						matchlen = ((int) source[off + src + 2]) & 0xff;
						src += 3;
					}
				}
				else
				...
				if (level == 1)
				{
					fetch = (int) fast_read(destination, 0, last_hashed + 1, 3); // destination[last_hashed
					                                                             // +
					                                                             // 1]
					                                                             // |
				...
				{
					fetch = (int) fast_read(source, off, src, 4);
				}
				last_hashed = dst - 1;
			}
			else
			{
				if (dst <= last_matchstart)
				{
					...
					if (level == 1)
					{
						...
						{
							last_hashed++;
							int fetch2 = (int) fast_read(destination, 0,
							        last_hashed, 3);
