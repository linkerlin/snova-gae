...
public class NetworkHelper
{
	...
	/**
	 * The SOMAXCONN value of the current machine. If failed to get the value,
	 * 3072 is used as a default value.
	 */
	...
	static
	{
		...
		{
			// The default local host names did not work. Try hard-coded IPv4
			// address.
			try
			{
			...
			{
				// The hard-coded IPv4 address did not work. Try hard coded IPv6
				// address.
				try
				{
					localhost = InetAddress.getByAddress(new byte[] { 0, 0, 0,
				...
				{
					throw new Error(
					        "Failed to resolve localhost - incorrect network configuration?",
					        e2);
				}
			}
		}
	...
	public static boolean isValidIpV6Address(String ipAddress)
	{
		...
		for (int i = 0; i < length; i++)
		{
			...
			// case for an open bracket [x:x:x:...x]
				...
				case '%':
					...
					{
						// right now we just support an integer after the % so
						// if
						// this is not
		...
		if (numberOfPeriods > 0)
		{
			// There is a test case with 7 colons and valid ipv4 this should
			// resolve it
			if (numberOfPeriods != 3
			        || !(isValidIp4Word(word.toString()) && numberOfColons < 7))
			{
				return false;
			}
		}
		else
		{
			...
			{
				if (word.length() == 0
				        && ipAddress.charAt(length - 1 - offset) == ':'
